...
class Time_Delay(nn.Module):
    def __init__(self, context, input_dim, output_dim, node_num, full_context):
        ...
        self.tdnn1 = tdnn.TDNN(context[0], 60, node_num[0], full_context[0])
        ...
        self.fc3 = nn.Linear(node_num[7], 107)
        
    def forward(self,x):
        a1 = F.relu(self.tdnn1(x))
        a2 = F.relu(self.tdnn2(a1))
        a3 = F.relu(self.tdnn3(a2))
        a4 = F.max_pool1d(F.relu(self.tdnn4(a3)), kernel_size = 3, stride = 3)
        a5 = F.max_pool1d(F.relu(self.tdnn5(a4)), kernel_size = 3, stride = 3)
        #a6 = self.statistic_pooling(a5)
        a6 = a5.view(a5.size()[0], -1)
        a7 = F.relu(self.fc1(a6))
        a8 = F.relu(self.fc2(a7))
        output = self.fc3(a8)
        return output
...
def train():
    ...
    net = Time_Delay(context, 24, 107, node_num, full_context)
    net = net.to(device)
    trans = transforms.Compose([transforms.ToTensor()])
    train_set = trainset.TrainSet('./all_feature/', transforms = trans)
    trainloader = DataLoader(train_set, batch_size = 100, num_workers = 20, shuffle = True)
...
def test(net):
    ...
    correct = 0
    trans = transforms.Compose([transforms.ToTensor()])
    test_set = testset.TestSet('./fb_feature/', transforms = trans)
    testloader = DataLoader(test_set, batch_size = 100, num_workers = 10, shuffle = True)
    torch.set_num_threads(10)
    with torch.no_grad():
